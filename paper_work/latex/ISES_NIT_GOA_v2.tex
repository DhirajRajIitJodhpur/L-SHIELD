...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\linewidth]{figures/fig1_attack_severity.pdf}
...
Because the VAE encoder compresses high-dimensional pixel structures into low-dimensional latent channels, adversarial perturbations manifest as localized statistical outliers along specific channel coordinates. Per-position statistical projection intercepts these localized spikes without modifying uncorrupted spatial positions, thereby preserving fine-grained semantic layout while eliminating adversarial energy. Calibration statistics stabilize rapidly with $N$, since per-position
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig7_qualitative_samples.pdf}
...
equally to a maximum score of $3.0$. As shown in Fig.\,\ref{fig:composite_quality}, L-SHIELD surpasses the clean baseline ceiling ($1.78$, dashed) on PGD, PhotoGuard, and C\&W. L-SHIELD is compared against
...
Table~\ref{tab:main_results} and Fig.\,\ref{fig:qualitative} present the complete defense performance comparison and visual output restoration. Across both datasets, all four attacks consistently degrade the Stable Diffusion img2img output, with PGD achieving the most severe structural damage (SSIM 0.354 on COCO-50 and 0.382 on COCO-50B). Fig.\,\ref{fig:qualitative} shows representative COCO-50 qualitative outputs; SSIM annotations are averages over all 50 images.
...
as macro-level anchors. The \textbf{+pp} (percentage points) annotations above L-SHIELD bars
quantify absolute ASR reduction relative to no-defense
($\Delta_{\mathrm{ASR}} = \mathrm{ASR}_{\text{no-def}} - \mathrm{ASR}_{\text{L-SHIELD}}$):
$+30$\,pp (percentage points) for C\&W ($98\%{\to}68\%$), $+4$\,pp for FGSM, and $+2$\,pp for
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.98\linewidth]{figures/fig5_cross_dataset.pdf}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\linewidth]{figures/fig3_fid_ssim_tradeoff.pdf}
...
As shown in Fig.\,\ref{fig:cross_dataset}, L-SHIELD achieves identical 14.4\% average recovery on both subsets without subset-specific tuning, versus DiffPure's 9.4\% (COCO-50) and 8.1\% (COCO-50B), with FID improvements of 17.4\% and 18.5\% respectively. Both subsets are drawn from COCO val2017, so this measures within-distribution robustness across disjoint image sets. The consistency reflects that per-position $(\mu,\sigma)$ calibration captures the VAE encoder's architecture-intrinsic latent structure rather than content-specific features of any individual image set.

\subsection{Computational Overhead and Latency Analysis}

A primary practical constraint of adversarial defenses in real-world generative workflows is inference latency and memory footprint. Pixel-domain purification defenses such as DiffPure~\cite{nie2022diffusion} require executing multi-step forward-reverse diffusion passes ($t=0.1T$), spanning dozens of iterative U-Net evaluations per image and introducing substantial computational overhead ($>2.5$\,s per image on high-end hardware).

In contrast, L-SHIELD operates as a lightweight, zero-overhead read-only lookup table $(\boldsymbol{\mu}, \boldsymbol{\sigma})$ during inference. At the $4\times64\times64$ VAE spatial resolution (16,384 total latent positions), per-position Z-score evaluation and 3-sigma clipping require only $O(hwc)$ elementwise operations ($81.9\,\text{KFLOPs}$). On modern GPU hardware, this elementwise tensor operation executes with sub-millisecond latency ($<0.15$\,ms) and an exact memory footprint of only 128\,KB ($0.125$\,MB) for storing float32 calibration statistics. This extreme computational and memory efficiency makes L-SHIELD uniquely suited for real-time safety-critical generative deployments where iterative purification is prohibitively expensive.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig8_threshold_sensitivity.pdf}
  \caption{Sensitivity of Defense Recovery Rate (\%) to Clipping Threshold Multiplier $\lambda \in [1.0, 5.0]$. L-SHIELD achieves optimal restoration at $\lambda = 3.0$ across all white-box attack types.}
  \label{fig:threshold_sensitivity}
\end{figure}

\subsection{Sensitivity to Clipping Threshold}

To evaluate defense sensitivity to hyperparameter choice, Fig.\,\ref{fig:threshold_sensitivity} plots Recovery\% across threshold multipliers $\lambda \in [1.0, 5.0]$. Across all four white-box attacks (PGD, C\&W, PhotoGuard, FGSM), the defense performance exhibits a smooth bell-shaped trajectory peaking precisely at $\lambda = 3.0$ (matching the main results in Table~\ref{tab:main_results}: 24.4\% for PGD, 18.1\% for C\&W, 10.0\% for PhotoGuard, and 5.1\% for FGSM). At tight threshold boundaries ($\lambda < 2.0$), overly aggressive clipping truncates valid high-variance clean activations, introducing structural distortion. Conversely, at loose threshold boundaries ($\lambda > 4.0$), unclipped adversarial outliers pass through to the U-Net denoiser, diminishing restoration efficacy. Setting $\lambda = 3.0$ corresponds to the standard three-sigma Gaussian boundary ($\approx 99.73\%$ clean activation preservation), providing the optimal analytical trade-off between clean feature fidelity and adversarial outlier suppression without requiring dataset-specific tuning. Recent studies~\cite{zheng2024latentguard,huang2024safelatent,wang2025advpurify,yang2024robustlatent} further corroborate that latent-domain statistical projection provides superior robustness over traditional pixel-level purification.
...
Both evaluated subsets are drawn from COCO val2017, demonstrating within-distribution robustness
across disjoint image sets but leaving cross-dataset generalization to distinct domains (e.g., ImageNet, medical modalities) as future work; FID evaluated at $n{=}50$ is indicative of distributional shifts.
L-SHIELD exhibits mild performance degradation on FGSM in COCO-50B ($-3.0\%$) due to the sparse latent
outlier pattern ($0.38\%$, Section~\ref{sec:vulnerability}), where statistical clipping has few anomalies to suppress. Furthermore, our white-box evaluation on Stable Diffusion v1.5 establishes latent bottleneck resilience; extending per-position statistical projection to adaptive black-box threat models and larger latent diffusion architectures (e.g., SDXL, Flux) represents an important direction for future research.

\enlargethispage{25pt}
\section{Conclusion}

We presented \textbf{L-SHIELD}, a training-free per-position latent clipping defense designed to protect latent diffusion models against white-box adversarial attacks. By intercepting adversarial corruption directly within the VAE bottleneck, L-SHIELD pre-computes per-position mean and standard deviation $(\boldsymbol{\mu}, \boldsymbol{\sigma})$ from clean encodings and clips inference latents to a 3-sigma boundary. On COCO-50, L-SHIELD achieves a 53\% recovery improvement over DiffPure ($14.4\%$ vs.\ $9.4\%$) and a 17.4\% FID reduction ($182.7$ vs.\ $221.1$), reproduced consistently on COCO-50B ($14.4\%$ recovery). Its zero-retraining design and sub-millisecond execution make L-SHIELD a practical, highly scalable defense for real-time generative image pipelines.
